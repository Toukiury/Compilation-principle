...
namespace ir {
    ...
    */
    void Scope::push(const std::string &name, std::shared_ptr<Value> value) {
        // 添加符号到当前作用域
            ...
            // 立即更新符号表文件
            dumpToFile("symbol_table.txt");
            
            // 打印调试信息
            LOG_INFO("Symbol %s (type: %s) added to scope level %d", 
                name.c_str(), 
                value->type_->print().c_str(),
                symbols_.size());
    }
    /**
     * @brief 从符号表中查找一个符号
    */
    ...
    void Scope::print() {
        std::cout << "Current scope symbol table\n";
        for (const auto &m : symbols_) {
            for (const auto &pair : m) {
                std::cout << "Symbol name:" << pair.first << " " << "Symbol address:" << pair.second->print() << "\n";
            }
        }
    }
    
    // 新增方法：将符号表写入文件
    void Scope::dumpToFile(const std::string &filename) {
        std::ofstream out(filename);
        if (!out.is_open()) {
            LOG_ERROR("Unable to open file %s write to symbol table", filename.c_str());
            return;
        }
        
        out << "Symbol table content (scope hierarchy from inside to out):\n";
        for (int i = int(symbols_.size()) - 1; i >= 0; i--) {
            if (symbols_[i].empty()) continue;  // 跳过空作用域
            out << "=== Scope level " << symbols_.size() - i << " ===\n";
            for (const auto &pair : symbols_[i]) {
                out << "Symbol name:" << pair.first 
                    << " | Type:" << pair.second->type_->print() 
                    << " | Value:" << pair.second->print() 
                    << " | Scopt:" << (i == 0 ? "Global" : "Local") << "\n";
            }
        }
        out.close();
        LOG_INFO("The symbol table has been written to the file %s", filename.c_str());
    }
        ...
        for (const auto &term : stmt.terms) {
            ...
            if (lhs && term.type != RelExprStmt::RelExprType::NULL_TYPE) {
                ...
                if(!CompareInst::can_be_compared(lhs->type_.get(), rhs->type_.get())) {
                    LOG_ERROR("Type mismatch in comparison expression, left:%s, right:%s", lhs->print().c_str(), rhs->print().c_str());
                    LOG_ERROR("Left type %s, Right type %s", lhs->type_->print().c_str(), rhs->type_->print().c_str());
                    throw common::IRGenException("Type mismatch in comparison expression");
                }
    
                if(op == Instruction::OpID::Null) {
                    LOG_ERROR("Unsupported comparison operator, left:%s, right:%s", lhs->print().c_str(), rhs->print().c_str());
                    throw common::IRGenException("Unsupported comparison operator");
                }
                // 新建指令，并赋予基本块信息
    ...
    void IRGenerator::visit(UnaryExprStmt &stmt) {
        ...
        for(auto &t: stmt.types) {
            ...
            if (op == Instruction::OpID::Null) {
                LOG_ERROR("Unsupported unary operator, expression:%s, operator: %d", val->print().c_str(), t);
                throw common::IRGenException("Unsupported unary operator");
            }
    ...
    void IRGenerator::visit(FuncCallStmt &stmt) {
        ...
        if(!val) {
            LOG_FATAL("Function not found: %s",stmt.id.c_str());
            throw common::IRGenException("Function not found");
        }
        assert(val != nullptr);
        ...
        if(args.size() != val->args_.size()) {
            LOG_ERROR("Argument count mismatch in function call, function:%s", stmt.id.c_str());
            throw common::IRGenException("Argument count mismatch in function call");
        }
        for (auto i = 0; i < val->args_.size(); i++) {
            if (!BinaryInst::can_compute(val->args_[i]->type_.get(), args[i]->type_.get())) {
                LOG_ERROR("Argument type mismatch in function call, function:%s", stmt.id.c_str());
                throw common::IRGenException("Argument type mismatch in function call");
            }
    ...
    void IRGenerator::visit(ConstDeclStmt &stmt) {
        for (const auto &pair : stmt.pairs) {
            ...
            if (value_stmt->type == ValueStmt::ValueType::Number) {
                ...
                LOG_ERROR("Unsupported type in constant declaration, name:%s, type:%d, pointer:%p", name.c_str(), value_stmt->type, value_stmt);
            }
            this->module_.add_literal(val);
            val->name_ = name;
            // 判断是否是全局作用域
    ...
    void IRGenerator::visit(LValStmt &stmt) {
        ...
        if (!val) {
            LOG_FATAL("There is no such symbol：%s",stmt.id.c_str());
        }
        assert (val != nullptr);
    ...
    void IRGenerator::visit(BreakStmt &stmt) {
        if(!this->scope_.is_in_loop()) {
            LOG_ERROR("The break statement is not in a loop");
            throw common::IRGenException("The break statement is not in a loop");
        }
    ...
    void IRGenerator::visit(ContinueStmt &stmt) {
        if(!this->scope_.is_in_loop()) {
            LOG_ERROR("The continue statement is not in a loop");
            throw common::IRGenException("The continue statement is not in a loop");
        }
    ...
    void IRGenerator::visit(ProgramStmt &stmt) {
        ...
        scope_.dumpToFile("symbol_table.txt");
        dumpIRToFile("ir.txt");
    }
    
    
    void IRGenerator::dumpIRToFile(const std::string& filename) {
        std::ofstream out(filename);
        if (!out.is_open()) {
            LOG_ERROR("Unable to open file %s write intermediate code", filename.c_str());
            return;
        }
        ...
        for (const auto& global : module_.global_identifiers_) {
            if (global->is_const_) {
                out << "const " << global->type_->print() << " " << global->name_ 
                    << " = " << global->init_val_->print() << ";\n";
            } else {
        ...
        // 输出函数
        for (const auto& func : module_.functions_) {
            out << "=== Function: " << func->name_ << " ===\n";
            out << func->print() << "\n";
            
            // 输出基本块
            for (const auto& bb : func->basic_blocks_) {
                out << "Basic blocks: " << bb->name_ << "\n";
        ...
        out.close();
        LOG_INFO("Intermediate code has been written to the file %s", filename.c_str());
    }
